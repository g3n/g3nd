...
func init() {
	TestMap["gui.checkradio"] = &CheckRadio{}
}
...
func (t *CheckRadio) Initialize(ctx *Context) {
	...
	cb1.Subscribe(gui.OnChange, func(name string, ev interface{}) {
		log.Info("Checkbox1 OnChange. State:%v", cb1.Value())
	})
	ctx.Gui.Add(cb1)
	...
	cb2.Subscribe(gui.OnChange, func(name string, ev interface{}) {
		log.Info("Checkbox1 OnChange. State:%v", cb2.Value())
	})
	ctx.Gui.Add(cb2)
	...
	cb4.Subscribe(gui.OnChange, func(name string, ev interface{}) {
		log.Info("Checkbox4 OnChange. State:%v", cb4.Value())
	})
	ctx.Gui.Add(cb4)
	...
	rb1.Subscribe(gui.OnChange, func(name string, ev interface{}) {
		log.Info("RadioButton 1 OnChange. State:%v", rb1.Value())
	})
	ctx.Gui.Add(rb1)
	...
	rb2.Subscribe(gui.OnChange, func(name string, ev interface{}) {
		log.Info("RadioButton 2 OnChange. State:%v", rb2.Value())
	})
	ctx.Gui.Add(rb2)
	...
	rb3.Subscribe(gui.OnChange, func(name string, ev interface{}) {
		log.Info("RadioButton 3 OnChange. State:%v", rb3.Value())
	})
	ctx.Gui.Add(rb3)
	...
	rb4.Subscribe(gui.OnChange, func(name string, ev interface{}) {
		log.Info("RadioButton 4 OnChange. State:%v", rb4.Value())
	})
	ctx.Gui.Add(rb4)
	// RadioButton 4 (radio group)
	...
	rb5.Subscribe(gui.OnChange, func(name string, ev interface{}) {
		log.Info("RadioButton 5 OnChange. State:%v", rb5.Value())
	})
	ctx.Gui.Add(rb5)
}

func (t *CheckRadio) Render(ctx *Context) {
}
