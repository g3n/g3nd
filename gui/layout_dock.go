...
func (t *GuiLayoutDock) Initialize(app *g3nd.App) {

	dl := gui.NewDockLayout()
	app.GuiPanel().SetLayout(dl)
	...
	app.GuiPanel().Add(right2)

	// Center panel
	center := gui.NewPanel(0, 0)
	center.SetLayoutParams(&gui.DockLayoutParams{Edge: gui.DockCenter})
	app.GuiPanel().Add(center)
}

func (t *GuiLayoutDock) Render(app *g3nd.App) {
}
