...
func (t *TextureCylinder) Initialize(app *g3nd.App) {
	...
	// Left cylinder
	tex, err := texture.NewTexture2DFromImage(app.DirData() + "/images/brick1.jpg")
	if err != nil {
	...
	// Middle cylinder
	tex, err = texture.NewTexture2DFromImage(app.DirData() + "/images/moss.png")
	if err != nil {
		app.Log().Fatal("Error loading texture: %s", err)
	}
	...
	// Right cylinder
	tex, err = texture.NewTexture2DFromImage(app.DirData() + "/images/checkerboard.jpg")
	if err != nil {
		app.Log().Fatal("Error loading texture: %s", err)
	}
...
func (t *TextureCylinder) Render(app *g3nd.App) {
