...
func (t *TankTest) Initialize(ctx *Context) {

	t.ctx = ctx
	ctx.Orbit.EnableKeys = false
	...
	t.velocity = 5.0
	t.rotvel = 0.8
	ctx.Scene.Add(t.model.node)

	// Subscribe to key events
	ctx.Win.Subscribe(window.OnKeyDown, t.onKey)
	ctx.Win.Subscribe(window.OnKeyUp, t.onKey)
}

func (t *TankTest) Render(ctx *Context) {

	if t.commands[CMD_LEFT] || t.commands[CMD_RIGHT] {
		// Calculates angle delta to rotate
		angle := t.rotvel * float32(t.ctx.TimeDelta.Seconds())
	...
	if t.commands[CMD_FORWARD] || t.commands[CMD_BACKWARD] {
		...
		dist := t.velocity * float32(t.ctx.TimeDelta.Seconds())
...
func (t *TankTest) newTankModel() *TankModel {
	...
	texfile := t.ctx.DirData + "/images/wheel.png"
